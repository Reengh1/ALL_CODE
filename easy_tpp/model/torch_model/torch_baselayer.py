...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, attn_weight = True):
        if self.use_residual:
            x, attn = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask, attn_weight))
            if self.feed_forward is not None:
                return self.sublayer[1](x, self.feed_forward), attn
            else:
                return x, attn
        else:
            x , attn = self.self_attn(x, x, x, mask, attn_weight)
            if self.feed_forward is not None:
                return self.feed_forward(x), attn
            else:
                return x, attn
