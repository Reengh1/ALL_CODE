...
from easy_tpp.model.torch_model.utlis import LinearPredictor, time_loss
...
class THP(TorchBaseModel):
    ...
    def forward(self, time_seqs, type_seqs, attention_mask, require_attn=False):
        """Call the model
        ...
        """
        #In Order to Get infectivity Matrix
    ...
    def loglike_loss(self, batch):
        ...
        # [batch_size, seq_len, hidden_size]
        enc_out, enc_att = self.forward(time_seqs[:, :-1], type_seqs[:, :-1], attention_mask[:, :-1, :-1], require_attn=True)
