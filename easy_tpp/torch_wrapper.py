...
from easy_tpp.utils.hcl_utils import event2seq_embedding, event_contrastive_loss, seq_contrastive_loss,sampling_positive_seqs,sampling_negative_seqs_random, build_attn_mask_from_nonpad
...
class TorchModelWrapper:
    ...
    def run_batch_hcl(self, batch, phase):
        batch = batch.to(self.device).values()
        if phase in (RunnerPhase.TRAIN, RunnerPhase.VALIDATE):
            # set mode to train
            is_training = (phase == RunnerPhase.TRAIN)
            ...
            if is_training:
                self.opt.zero_grad()
                (loss+pred_loss+se/100+0.2*nce1+12*nce2).backward()
                self.opt.step()
            return loss, pred_loss, se, pred_num_event, batch_num_pred, num_event
    def run_batch_dhcl(self, batch, phase):
        batch = batch.to(self.device).values()
        if phase in (RunnerPhase.TRAIN, RunnerPhase.VALIDATE):
            # set mode to train
            is_training = (phase == RunnerPhase.TRAIN)
